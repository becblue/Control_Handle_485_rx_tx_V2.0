...
u8 CRC_Calc_Header(u8 dataValue,u8 crcValue)
{
    return HeaderCRC[crcValue ^ dataValue];
}

u16 CRC_Calc_Data(u8 dataValue,u16 crcValue)
{
...
u8 BacnetCRC8(u8 *databuf,u8 datalen)
{
    volatile u8 crc = 0xff; /* accumulates the crc value */
		u8 i=0;
		u8 * data;
		data=databuf;
		for(;i<datalen;i++)
		{
				crc = CRC_Calc_Header(*data, crc);
				data++;
		}
		crc=~crc;
		return crc;
...
u16 BacnetCRC16(u8 *databuf,u8 datalen)
{
    volatile u16 crc = 0xffff;		
		u8 i=0;
		u8 * data;
		data=databuf;
		for(;i<datalen;i++)
		{
				crc = CRC_Calc_Data(*data, crc);
				data++;
		}
		crc=~crc;
		return crc;
...
u16 Modbus_CRC_16(u8 *puchMsg, u8 count)  //????crcD??é??
{
	u8 i, j;
	u8 XORResult;
	u16 xorCRC = 0xA001; //éè??CRCµ??à??ê? //
	u16 Verify_Data = 0xffff;
	for (i = 0; i < count; i++)
	{
		Verify_Data ^= puchMsg[i]; 	   //??0xffffoí8??êy?Y?àòì?ò
		for (j = 0; j < 8; j++)
		{
			XORResult = Verify_Data & 1; 	 //?D????òaò?3yµ?ò???ê?·??aò?
			Verify_Data >>= 1;
			if (XORResult) 	   //ò?3y???a1?ò??crcó??à??ê??àó?
			{
				Verify_Data ^= xorCRC;
			}
		}
	}
	return Verify_Data;			//·µ??16??µ?crcD??é??
}
